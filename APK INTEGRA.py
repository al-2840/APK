...
Versi (update) : 4.2.20
Link Repo      : https://github.com/al-2840/APK/blob/main/APK.py
"""
import sys
import time
import ast
...
def find_warehouse(query: str):
    ...
    for wh in store.warehouses.values():
        # Cocokkan ID persis
        if wh.warehouse_id.lower() == q:
            return wh
        # Cocokkan nama persis
        if wh.name.lower() == q:
            return wh
        # Cocokkan sebagian nama (substring)
        if q in wh.name.lower():
            return wh
    return None
...
def print_stock_per_warehouse(wh_id=None):
    ...
    for w in gudangs:
        ...
        if not w.stock:
            ...
            print(f"{'ID':<8} | {'Nama':<20} | {'Jumlah':<6} | {'Harga Beli':<15} | {'Kategori':<15} | {'Kondisi':<12}")
            print("-"*90)
            for item_id, data in w.stock.items():
                ...
                if not item:
                    ...
                    nama = item.name
                    harga = format_rupiah(item.price)   # ✅ konsisten dengan nota
                    kategori = KATEGORI_LIST.get(str(item.category), item.category)
                ...
                print(f"{item_id:<8} | {nama:<20} | {qty:<6} | {harga:<15} | {kategori:<15} | {status_colored:<12}")
...
def print_warehouses():
    ...
    # Header tabel
    print(f"{'ID':<8} | {'Nama Gudang':<20} | {'Alamat':<30} | {'Default':<10}")
    print("-"*75)

    # Isi tabel
    default_wh = getattr(store, "default_warehouse", "WH01")
    for w in store.warehouses.values():
        mark = "Ya" if w.warehouse_id == default_wh else ""
        print(f"{w.warehouse_id:<8} | {w.name:<20} | {w.address:<30} | {mark:<10}")

def lihat_pending_requests():
    print("\n\033[34m>> PERMINTAAN PENDING DARI SERVIS:\033[0m")
    print("\033[33m(pisahkan dengan koma, ketik 'all' untuk semua, Enter/0 = batal)\033[0m")
    ...
    try:
        ...
        for idx, line in enumerate(lines, 1):
            ...
            status = req['status']
            if status.lower() in ["siap jual", "ready"]:
                status_colored = f"\033[32m{status}\033[0m"
            elif status.lower() == "pending":
                status_colored = f"\033[33m{status}\033[0m"
            else:
            ...
            print(f"{idx:<4} | {req['date']:<20} | {req['item_id']:<10} | {req['qty_requested']:<5} | {req['operator']:<15} | {status_colored:<20}")

        # Input pilihan
        user_input = input("\nPilih nomor permintaan untuk approve: ").strip()
        if user_input == "" or user_input == "0":
            print("\033[33mDibatalkan oleh user.\033[0m")
            return

        if user_input.lower() == "all":
            choices = list(range(1, len(lines)+1))
        else:
        ...
        for idx, line in enumerate(lines, 1):
            ...
            if idx in choices:
                ...
                if item and item.confirmed_stock > 0 and item.price > 0:
                    ...
                    # catat log
                    log_inventory("Konfirmasi Siap Jual", item, {"qty": req['qty_requested'], "operator": req['operator']})
                    log_employee_action(current_user, current_role, "Approve Pending Request", {
                    ...
                    # pindahkan request ke approved_requests.txt
                    with open("approved_requests.txt", "a", encoding="utf-8") as af:
                        af.write(json.dumps(req) + "\n")

                    # update waiting_services.txt
                    try:
                        ...
                        for wline in waiting_lines:
                            ...
                            if req['item_id'] in srv.get('parts', []):
                                srv['status'] = "Ready"
...
def konfirmasi_stok_gudang():
    ...
    for item in items:
        ...
        # Update stok gudang (lengkap dengan kategori & harga)
        ...
        log_inventory(
            "Audit Stok Barang",
            item,
            {"warehouse": wh_id, "old_qty": old_qty, "new_qty": qty_real}
        )

        print(f"\n\033[32mStok {item.name} di {wh_id} dikonfirmasi: {old_qty} → {qty_real} unit\033[0m")

# Simpan/muat
def save_inventory(filename="inventory.json"):
    data = [asdict(item) for item in store.inventory.values()]
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            ...
            for d in data:
                ...
                store.inventory[item.item_id] = item
    except FileNotFoundError:
        print("\033[33mFile inventory belum ada, mulai dengan kosong.\033[0m")
...
def review_requests():
    ...
    print("\033[33m\033[3m(pisahkan dengan koma, ketik 'all' untuk semua, Enter/0=keluar)\033[0m")
    ...
    try:
        ...
        # Header tabel
        print(f"{'No':<4} | {'Tanggal':<12} | {'User':<12} | {'Barang':<20} | {'ID':<8} | {'Status':<12}")
        print("-"*80)
        ...
        for idx, line in enumerate(lines, 1):
            ...
            print(f"{idx:<4} | {req['date']:<12} | {req['user']:<12} | {req['name']:<20} | {req['item_id']:<8} | {status:<12}")

        # Input pilihan
        user_input = input("\nPilih nomor untuk approve: ").strip()
        if user_input == "" or user_input == "0":
            print("\n\033[33mDibatalkan oleh user.\033[0m")
            return

        if user_input.lower() == "all":
            choices = list(range(1, len(lines)+1))  # semua nomor
        else:
            choices = [int(x.strip()) for x in user_input.split(",") if x.strip().isdigit()]
        ...
        for idx_choice in choices:
            ...
            if item and item.confirmed_stock > 0 and item.price > 0 and item.status not in ["Rusak", "Hilang"]:
                item.status = "Siap Jual"
                approved.append(req['item_id'])
                print(f"\n\033[32mBarang {item.item_id} ({item.name}) dikonfirmasi SIAP JUAL oleh Admin\033[0m")
                save_inventory()
                log_employee_action(current_user, current_role, "Approve Barang", {
                    "item_id": item.item_id,
                    "name": item.name,
                    "reason": "Konfirmasi Siap Jual"
                })
            else:
                print(f"\n\033[31mBarang {req['name']} tidak valid (stok/harga/status belum memenuhi syarat)\033[0m")
...
def transaksi_penjualan():
    ...
    while True:
        query = input("\nFormat: Nama Barang (jumlah): ").strip()
        if query == "":
            break
        if "(" in query and ")" in query:
            try:
                name_part, qty_part = query.split("(")
                name = name_part.strip()
                qty = int(qty_part.replace(")", "").strip())
            except ValueError:
                print("\033[31mFormat salah. Gunakan: Nama Barang (jumlah)\033[0m")
                continue
        else:
            print("\033[31mFormat salah. Gunakan: Nama Barang (jumlah)\033[0m")
            continue
        ...
        if isinstance(item, InventoryItem):
            ...
            if qty > item.confirmed_stock:
                print(f"\nStok {item.name} tidak cukup (tersedia {item.confirmed_stock})")
                continue
            ...
            for obj in item:
                if obj.name.lower() == name.lower():
                    ...
                    if qty > obj.confirmed_stock:
                        print(f"\nStok {obj.name} tidak cukup (tersedia {obj.confirmed_stock})")
                        continue
    ...
    for i, q in cart:
        ...
        log_warehouse("OUT", i, from_wh=wh_id, qty=q, extra={"operator": current_user})
...
def riwayat_penjualan(sales_data):
    print("\n\033[34m>> RIWAYAT PENJUALAN BARANG:\033[0m")
    if not sales_data:
        print("\033[31m- KOSONG -\033[0m")
        return

    # kalau dict, ambil values
    if isinstance(sales_data, dict):
        sales_iter = sales_data.values()
    else:
        sales_iter = sales_data
    ...
    # Isi tabel
    for idx, sale in enumerate(sales_iter, 1):
        detail = "; ".join(
...
def tampilkan_nota_service(record, jasa, parts_cost, total_fee):
    print("\n\033[30m=== NOTA SERVICE PT.INTEGRA LOKOMOTIF ===\033[0m")
...
def tampilkan_nota_makanan(order: "FoodOrder", store: "Store"):
    print("\n\033[30m=== NOTA PEMBELIAN MAKANAN PT.INTEGRA \033[0m\n")
    ...
    print("\n\033[30m=== Terima kasih telah berbelanja di PT.INTEGRA \033[0m")

def tampilkan_nota_rental(record):
    print("\n\033[30m=== NOTA RENTAL PT.INTEGRA \033[0m")
    print(f"Customer      : {record.customer}")
    print(f"Lokomotif     : {record.lokomotif_id}")
...
def print_ready_stock():
    ...
    if ready_list:
        ...
        print(f"{'Nama Barang':<25} | {'Harga':>15} | {'Stok Aman':>10} | {'Gudang Asal':<20}")
        print("-"*80)
        for item in ready_list:
            gudang_list = [w.name for w in store.warehouses.values() if item.item_id in w.stock]
            gudang_ui = ", ".join(gudang_list) if gudang_list else "-"
            print(f"{item.name:<25} | {format_rupiah(item.price):>15} | {item.confirmed_stock:>10} | {gudang_ui:<20}")

    elif waiting_list:
        print("\n\033[33m>> Barang menunggu konfirmasi gudang:\033[0m")
        print(f"{'Nama Barang':<25} | {'Stok Gudang':<15}")
        print("-"*45)
        for _, name, qty in waiting_list:
            print(f"{name:<25} | {qty:<15}")
...
def tambah_akun():
    ...
    print("\n>> Nama Karyawan (belum punya akun):")
    available = []
    for emp_id, emp in store.employees.items():
        if not any(u["employee_id"] == emp_id for u in users.values()):
            print(f"{emp_id:<8} | {emp.name:<20} | {emp.role:<10} | Status: {getattr(emp,'status','Menunggu Aktivasi')}")
    ...
    if not username:
        ...
        return
    if username in users:
        print(f"\n\033[31mUsername {username} sudah ada\033[0m")
        return
    ...
    # Simpan akun dengan status default Menunggu Aktivasi
    users[username] = {
        "password": password,
        "employee_id": emp_id,
        "role": role,
    ...
    print("\033[33mAkun masih menunggu aktivasi oleh Admin di menu 'Status Akun'\033[0m")
...
def ubah_role():
    ...
    old_role = users[username].get("role", "N/A")
    print(f"Role lama akun '{username}': {old_role}\n\033[3m\033[33m*enter jika ingin batal\033[0m")
    new_role = input("\nMasukkan role baru: ").strip().lower()

    if new_role:
        if new_role == "pembeli":
            print("\n\033[31mRole 'Pembeli' tidak bisa diberikan ke karyawan\033[0m")
            return
...
def hapus_akun_dan_karyawan():
    global current_user, users
    daftar = list_accounts(simple=False, return_list=True)
    if not daftar:
        return

    choice = input("\nMasukkan nomor akun yang ingin dihapus (Enter = batal): ").strip()
    if not choice.isdigit():
        print("\033[33mDibatalkan.\033[0m")
        return

    idx = int(choice)
    if idx < 1 or idx > len(daftar):
        print("\033[31mNomor tidak valid\033[0m")
        return

    username = daftar[idx-1]
    emp_id = users[username].get("employee_id")
    emp = store.employees.get(emp_id)

    # validasi status
    emp_status = emp.status if emp else users[username].get("status", "Aktif")
    if emp_status.lower() not in ["nonaktif", "keluar", "meninggal"]:
        print("\n\033[31mAkun hanya bisa dihapus jika status karyawan Nonaktif/Keluar/Meninggal\033[0m")
        return

    # alasan penghapusan
    alasan_ops = {"1": "Resign", "2": "PHK", "3": "Meninggal", "4": "Lainnya"}
    ...
    confirm = input(f"\nYakin hapus akun {username} dan karyawan {emp_id}? (y/n): ").lower()
    if confirm == "y":
        # catat log
        record = {
            "date": datetime.now().strftime("%Y-%m-%d %H:%M"),
            ...
            "reason": alasan,
            "note": note}
        with open("akun_karyawan_dihapus.txt", "a", encoding="utf-8") as f:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")

        # hapus akun
        del users[username]
        save_users()
        print(f"\033[32mAkun '{username}' berhasil dihapus\033[0m")

        # hapus karyawan
        if emp_id in store.employees:
            del store.employees[emp_id]
            save_employees()
            print(f"\033[32mKaryawan '{emp_id}' ikut dihapus\033[0m")

        # logout jika akun sendiri
...
def forgot_password():
    ...
    users[username]["temporary"] = True
    save_users()

    print(f"\n\033[32mPassword akun '{username}' direset ke sementara: {temp_pass}\033[0m")
    print("\033[33mUser wajib login dengan password sementara lalu mengganti password sendiri\033[0m")

def list_accounts(simple=False, return_list=False):
    ...
    daftar = []
    if simple:
        print(f"\n{'No':<4} | {'Username':<15} | {'Role':<12} | {'Employee ID':<12} | {'Nama':<20} | {'Last Login':<20}")
        print("-"*95)
        for idx, uname in enumerate(sorted(users.keys()), 1):
            data = users[uname]
            role = data.get("role", "-")
            ...
            daftar.append(uname)
            print(f"{idx:<4} | {uname:<15} | {role:<12} | {emp_id:<12} | {emp_name:<20} | {last_login:<20}")
    else:
        print(f"\n{'No':<4} | {'Username':<15} | {'Role':<12} | {'ID (EMP)':<10} | {'Nama Karyawan':<20} | {'Status':<23} | {'Last Login':<20}")
        print("-"*105)
        for idx, uname in enumerate(sorted(users.keys()), 1):
            data = users[uname]
            emp_id = data.get("employee_id", "N/A")
            emp_name = store.employees[emp_id].name if emp_id in store.employees else "Unknown"
            role = data.get("role", store.employees[emp_id].role if emp_id in store.employees else "N/A")
            ...
            elif status.lower() in ["nonaktif", "keluar"]:
                status_display = f"\033[31m{status}\033[0m"
            elif status.lower() in ["pending", "cuti"]:
                status_display = f"\033[33m{status}\033[0m"
            else:
                status_display = status

            daftar.append(uname)
            print(f"{idx:<4} | {uname:<15} | {role:<12} | {emp_id:<10} | {emp_name:<20} | {status_display:<23} | {last_login:<20}")
...
def edit_profil(emp_id, current_user, current_role):
    ...
    new_address = input(f"Alamat ({getattr(emp,'address','-')}): ").strip()
    if new_address:
        emp.address = new_address

    new_phone = input(f"Nomor HP ({getattr(emp,'phone','-')}): ").strip()
    if new_phone:
        emp.phone = new_phone
    ...
    if current_role == "admin":
        ...
        if new_role and new_role.lower() != "pembeli":
            emp.role = new_role
            for uname, data in users.items():
                if data.get("employee_id") == emp_id:
                    data["role"] = new_role
...
def set_account_status():
    ...
    choice = input("\nMasukkan nomor akun yang ingin diubah statusnya (Enter = batal): ").strip()
    if not choice.isdigit():
        print("\033[33mDibatalkan.\033[0m")
        return
    ...
    if current_status == "Aktif":
        ...
        if yakin == "y":
            ...
            if emp_id in store.employees:
                store.employees[emp_id].status = "Nonaktif"
            save_users()
            save_employees()
            ...
            with open("status_changes.txt", "a", encoding="utf-8") as f:
                f.write(json.dumps(record, ensure_ascii=False) + "\n")
    else:
        ...
        if yakin == "y":
            ...
            if emp_id in store.employees:
                store.employees[emp_id].status = "Aktif"
            save_users()
            save_employees()
...
def update_last_login(uname: str):
    if uname in users:
        ...
        with open("login_history.txt", "a", encoding="utf-8") as f:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")

def request_password_reset():
    global current_user, users
    print("\n\033[44m=========== PERMINTAAN RESET PASSWORD ===========\033[0m")
    ...
    with open("password_requests.txt", "a", encoding="utf-8") as f:
        f.write(json.dumps(record, ensure_ascii=False) + "\n")

    print(f"\n\033[33mPermintaan reset password untuk akun '{username}' telah dikirim ke Admin.\033[0m")
    print("\033[33mTunggu password sementara dari Admin.\033[0m")

def process_password_requests():
    global users
    print("\n\033[44m=========== PROSES PERMINTAAN RESET PASSWORD ===========\033[0m")

    try:
        with open("password_requests.txt", "r", encoding="utf-8") as f:
            lines = f.readlines()
    except FileNotFoundError:
        print("\033[31mTidak ada permintaan reset password\033[0m")
        return

    if not lines:
        print("\033[33mTidak ada permintaan reset password\033[0m")
        return

    print(f"{'No':<4} | {'Tanggal':<20} | {'Username':<15} | {'Status':<10}")
    print("-"*55)
    for idx, line in enumerate(lines, 1):
        req = json.loads(line.strip())
        print(f"{idx:<4} | {req['date']:<20} | {req['username']:<15} | {req['status']:<10}")

    choice = input("\nPilih nomor permintaan untuk diproses (Enter= batal): ").strip()
    if not choice.isdigit():
        print("\033[33mDibatalkan.\033[0m")
        return
    ...
    req = json.loads(lines[idx-1].strip())
    ...
    req["status"] = "Selesai"
    lines[idx-1] = json.dumps(req) + "\n"
    with open("password_requests.txt", "w", encoding="utf-8") as f:
        f.writelines(lines)
...
def force_change_password(username):
    ...
    users[username]["password"] = new_pass
    users[username]["temporary"] = False   # hapus flag password sementara
    save_users()
    ...
    print(f"\n\033[32mPassword akun '{username}' berhasil diganti permanen\033[0m")
...
def login():
    ...
    while True:
        ...
        if u in users and users[u]["password"] == p:
            # cek status akun
            if users[u].get("status", "Aktif") != "Aktif":
                print("\n\033[31mAkun Anda belum aktif. Hubungi admin untuk aktivasi.\033[0m")
                return
            ...
            # cek apakah password sementara
            if users[u].get("temporary", False):
                print(f"\n\033[33mAnda login dengan password sementara. Silakan ganti password permanen sekarang.\033[0m")
                force_change_password(u)
                return
...
def add_employee(name: str, role: str, address: str = "", phone: str = "", status: str = "Aktif", silent=False):
    # Cek duplikasi nama
    for emp in store.employees.values():
    ...
    store.employees[emp_id].status = status   # simpan status di objek Employee juga
    ...
    if not silent:
        ...
        print(f"\033[33mAkun login: username='{username}', password='{password}' (role: {role}, status: {status})\033[0m")
...
def add_warehouse(name: str, address: str, silent=False):
    # cek duplikat nama gudang
    for w in store.warehouses.values():
        if w.name.lower() == name.lower():
            print(f"\n\033[31mGudang dengan nama '{name}' sudah ada (ID: {w.warehouse_id})\033[0m")
            return None

    # generate ID unik (misalnya W001, W002, dst)
    wid = f"W{len(store.warehouses) + 1:03}"

    # buat objek Warehouse dengan ID, nama, dan alamat
    store.warehouses[wid] = Warehouse(wid, name, address)

    # log otomatis
    log_warehouse("Tambah Gudang", item_id="N/A", extra={"id": wid, "name": name, "address": address})

    if not silent:
        print(f"\n\033[32mGudang {name} ditambahkan dengan ID {wid}\033[0m")

    return store.warehouses[wid]
...
def hapus_edit_transaksi():
    ...
    # Ambil semua transaksi sebagai list
    sales_list = list(store.sales.values())
    ...
    if idx_choice == 0:
        return
    if idx_choice > len(sales_list):
        print("\n\033[31mNomor tidak valid\033[0m")
        return

    sale = sales_list[idx_choice - 1]
    action = input("Hapus atau Edit? (h/e): ").strip().lower()

    if action == "h":
        # hapus dari dict berdasarkan sale_id
        del store.sales[sale.sale_id]
    ...
    while True:
        ...
        if current_role == "gudang":
            ...
            # Tambah barang
                ...
                # Pilih gudang tujuan
                print_warehouses()
                wh_input = input("\nMasukkan ID/Nama gudang (Enter = default): ").strip()
                if not wh_input:
                    default_wh = getattr(store, "default_warehouse", "WH01")
                    warehouse = store.warehouses.get(default_wh)
                    if not warehouse:
                        warehouse = Warehouse(default_wh, "Gudang Utama", "-")
                        store.warehouses[default_wh] = warehouse
                else:
                    warehouse = find_warehouse(wh_input)
                    if not warehouse:
                        print("\033[31mGudang tidak ditemukan, barang masuk ke gudang default\033[0m")
                        default_wh = getattr(store, "default_warehouse", "WH01")
                        warehouse = store.warehouses.get(default_wh)
                        if not warehouse:
                            warehouse = Warehouse(default_wh, "Gudang Utama", "-")
                ...
                warehouse.stock[item_id] = {
                    "qty": jumlah,
                    "status": "Pending",
                    "price": harga_beli,
                    "category": category_code}

                log_warehouse("Barang Masuk Gudang", item_id, extra={"warehouse": warehouse.warehouse_id, "qty": jumlah, "price": harga_beli})
                print(f"\n\033[32mBarang {name} otomatis ditambahkan ke gudang {warehouse.warehouse_id}\033[0m")

                # Log inventaris
                log_inventory("Tambah Barang", store.inventory[item_id],
                            {"jumlah": jumlah, "harga": harga_beli, "kategori": category_code, "status": "Pending"})
                log_employee_action(current_user, current_role, "Tambah Barang",
                                    {"item": name, "qty": jumlah})
            ...
            # Minta konfirmasi siap jual
                ...
                print_inventory()
                item_query = input("\nID/Nama (gunakan koma, ketik 'all' untuk semua): ").strip()
                if item_query.lower() == "all":
                    items = list(store.inventory.values())
                else:
                    items = find_item(item_query)
                if not items:
                    print(f"\n\033[31mBarang {item_query} tidak ditemukan\033[0m")
            ...
            # Lihat Pending Req
            elif choice == "6": lihat_pending_requests()
            elif choice == "9":
                return
        ...
        # Bagian Admin
            ...
            if choice == "1": riwayat_barang()
            elif choice == "2": review_requests()
            elif choice == "9":
                return
...
def warehouse_menu():
    ...
    while True:
        ...
        # bagian admin
            ...
            ops = {
                "1": "Daftar gudang",
                "2": "Tambah gudang", # buat opsi klo enter batal
            ...
            # Tambah gudang
                ...
                name = input("\nNama gudang: ").strip()
                if not name:
                    print("\033[31mInput dibatalkan\033[0m")
                    continue
...
# KARYAWAN (clear v.4.1.16)
def employee_menu():
    valid_roles = ["Admin", "Kasir", "Service", "Rental", "Dapur", "Gudang"]
    status_ops = {"1": "Aktif", "2": "Nonaktif", "3": "Cuti"}
    while True:
        h = head("KELOLA KARYAWAN")
        ops = {
            "1": "Lihat Karyawan",
            "2": "Tambah Karyawan",
            "3": "Ubah Jabatan/Status",
            "4": "Hapus Karyawan",
            "5": "Cari Karyawan",
            "0": "Kembali"}
        ...
        # Lihat Karyawan
        if choice == "1":
            print("\n\033[34m>> DAFTAR KARYAWAN:\033[0m")
            if not store.employees:
                print("\033[31m- KOSONG -\033[0m")
            else:
                # Header tabel
                print(f"{'ID (EMP)':<8} | {'Nama':<20} | {'Role':<12} | {'Alamat':<30} | {'Telp':<15} | {'Status':<10}")
                print("-"*105)
                # Isi tabel
                for eid, emp in store.employees.items():
                    ...
                    print(f"{eid:<8} | {emp.name:<20} | {emp.role:<12} | {emp.address:<30} | {emp.phone:<15} | {status_colored:<10}")

            pause()
        # Tambah Karyawan
        elif choice == "2":
            print("\n\033[34m>> Tambah Karyawan\033[0m")
            print("\033[3m*enter = batal\033[0m")
            # Input nama
            name = input("\nNama: ").strip()
            if not name:
                print("\033[33mDibatalkan. Nama tidak diisi.\033[0m")
                continue

            # Pilihan role dengan angka
            print("\nPilih Jabatan:")
            for i, r in enumerate(valid_roles, 1):
                print(f"{i}. {r}")
            role_choice = input("Nomor jabatan: ").strip()
            if not role_choice:
                print("\033[33mDibatalkan. Jabatan tidak dipilih.\033[0m")
                continue
            if not role_choice.isdigit() or int(role_choice) < 1 or int(role_choice) > len(valid_roles):
                print("\033[31mPilihan jabatan tidak valid\033[0m")
                continue
            ...
            if not address:
                print("\033[33mDibatalkan. Alamat tidak diisi.\033[0m")
                continue

            # Input telepon
            phone = input("No. Telepon: ").strip()
            if not phone:
                print("\033[33mDibatalkan. Nomor telepon tidak diisi.\033[0m")
                continue

            # Status default → Menunggu Aktivasi
            add_employee(name, role, address, phone, status="Menunggu Aktivasi")
            save_employees()

            print(f"\n\033[32mKaryawan {name} berhasil ditambahkan sebagai {role}\033[0m")
            print("\033[33mAkun belum aktif. Perlu aktivasi oleh Admin di \033[32mmanajemen akun \033[33msebelum bisa digunakan.\033[0m")
        # Ubah Jabatan/Status
        elif choice == "3":
            print_employees()
            keyword = input("\nID/Nama karyawan (Enter = batal): ").strip().upper()
            if not keyword:
                print("\033[33mDibatalkan oleh user.\033[0m")
                continue
            ...
            if keyword in store.employees:
                target_id = keyword
            else:
                for eid, emp in store.employees.items():
                    if emp.name.upper() == keyword:
                        target_id = eid
                        break
            ...
            emp = store.employees[target_id]

            # Ubah nama
            new_name = input(f"Nama baru (Enter = tetap {emp.name}, ketik '-' untuk batal): ").strip()
            if new_name == "-":
                print("\033[33mEdit dibatalkan.\033[0m")
                continue
            if new_name:
                emp.name = new_name

            # Ubah role
            print("\nPilih Jabatan baru (Enter = tetap):")
            for i, r in enumerate(valid_roles, 1):
                print(f"{i}. {r}")
            role_choice = input("Nomor jabatan: ").strip()
            if role_choice:
                if role_choice.isdigit() and 1 <= int(role_choice) <= len(valid_roles):
                    emp.role = valid_roles[int(role_choice) - 1]
                else:
                    print("\033[31mPilihan jabatan tidak valid. Role tidak diubah.\033[0m")

            # Ubah status
            print("\nPilih Status baru (Enter = tetap):")
            for i, s in status_ops.items():
                print(f"{i}. {s}")
            status_choice = input("Nomor status: ").strip()
            if status_choice:
                if status_choice in status_ops:
                    emp.status = status_ops[status_choice]
                else:
                    print("\033[31mPilihan status tidak valid. Status tidak diubah.\033[0m")

            save_employees()
            print(f"\n\033[32mData karyawan {target_id} diperbarui\033[0m")
        # Hapus Karyawan
        elif choice == "4": hapus_akun_dan_karyawan()
        # Cari Karyawan
        elif choice == "5":
            keyword = input("\nID/Nama Karyawan (Enter = batal): ").strip().lower()
            if not keyword:
                print("\033[33mDibatalkan oleh user.\033[0m")
                continue

            found = False
            print(f"{'ID (EMP)':<8} | {'Nama':<20} | {'Role':<12} | {'Alamat':<25} | {'Telp':<15} | {'Status':<10}")
            print("-"*100)
            for eid, emp in store.employees.items():
                if keyword in eid.lower() or keyword in emp.name.lower():
                    ...
                    print(f"{eid:<8} | {emp.name:<20} | {emp.role:<12} | {emp.address:<25} | {emp.phone:<15} | {status_colored:<10}")
                    found = True
            if not found:
                print(f"\n\033[31mKaryawan '{keyword}' tidak ditemukan\033[0m")
            pause()
...
def account_menu():
    ...
    while True:
        ...
        if current_role == "admin":
            if choice == "1": list_accounts()
            elif choice == "2": tambah_akun()
            elif choice == "3": ubah_role()
            elif choice == "4": hapus_akun_dan_karyawan()
            elif choice == "5":
                keyword = input("\nID/Nama: ").strip()
                if not keyword:
                ...
                if target_id:
                    edit_profil(target_id, current_user, current_role)
                else:
                    print(f"\n\033[31mKaryawan '{keyword}' tidak ditemukan\033[0m")
            elif choice == "6":
                # Submenu password
                pw_ops = {
                    ...
                    "3": "Proses Permintaan Reset",
                    "0": "Kembali"}
                sub_choice = input_menu("Manajemen Password", pw_ops)
                if sub_choice == "1": reset_password()
                elif sub_choice == "2": forgot_password()
                elif sub_choice == "3": process_password_requests()
            elif choice == "7": set_account_status()
            elif choice == "8":
        ...
        # Karyawan menu
            ...
            if choice == "1":
                lihat_profil(users[current_user]["employee_id"], current_user, current_role)
            elif choice == "2":
...
def sales_menu():
    while True:
        ...
        if p == "1":
            print_ready_stock()
            pause()
        # Transaksi penjualan (hanya kasir)
        elif p == "2" and current_role != "admin": transaksi_penjualan()
        # Riwayat penjualan
        elif (p == "2" and current_role == "admin") or (p == "3" and current_role != "admin"):
            if current_role == "admin":
                riwayat_penjualan(store.sales.values())
            else:
                own_sales = [s for s in store.sales.values() if s.cashier == current_user]
                riwayat_penjualan(own_sales)
            pause()
        # Hapus/Edit transaksi (khusus admin)
        elif p == "3" and current_role == "admin": hapus_edit_transaksi()
        elif p == "0":
            return
    ...
    while True:
        ...
        # Riwayat
            ...
            if not store.services:
                ...
                print("\n\033[34m>> RIWAYAT SERVICE:\033[0m")

                # Header tabel
                print(f"{'No':<4} | {'Customer':<20} | {'Lokomotif':<20} | {'Jenis':<15} | {'Durasi':<8} | {'Mulai':<12} | {'Selesai':<12} | {'Biaya':<15}")
                print("-"*115)

                # Isi tabel
                for idx, r in enumerate(store.services, 1):
                    lok = store.lokomotif.get(r.lokomotif_id)
                    lok_name = lok.name if lok else r.lokomotif_id
                    print(f"{idx:<4} | {r.customer:<20} | {lok_name:<20} | {r.service_type:<15} | {str(r.days)+' hari':<8} | {r.start_date:<12} | {r.end_date:<12} | {format_rupiah(r.total_fee):<15}")
            pause()
        # Hapus lokomotif
        elif p == "5":
...
def start_menu():
    global current_user, current_role
    while True:
        head("SELAMAT DATANG DI APLIKASI INTEGRA")
